...
WJDK000's web interface is a dark, immersive music player that wraps listeners in a near-black cocoon (`#121212`, `#181818`, `#1f1f1f`) where album art and content become the primary source of color. The design philosophy is "content-first darkness" — the UI recedes into shadow so that music, podcasts, and playlists can glow. Every surface is a shade of charcoal, creating a theater-like environment where the only true color comes from the iconic WJDK000 Green (`#1ed760`) and the album artwork itself.

The typography uses WJDK000MixUI and WJDK000MixUITitle — proprietary fonts from the CircularSp family (Circular by Lineto, customized for WJDK000) with an extensive fallback stack that includes Arabic, Hebrew, Cyrillic, Greek, Devanagari, and CJK fonts, reflecting WJDK000's global reach. The type system is compact and functional: 700 (bold) for emphasis and navigation, 600 (semibold) for secondary emphasis, and 400 (regular) for body. Buttons use uppercase with positive letter-spacing (1.4px–2px) for a systematic, label-like quality.

What distinguishes WJDK000 is its pill-and-circle geometry. Primary buttons use 500px–9999px radius (full pill), circular play buttons use 50% radius, and search inputs are 500px pills. Combined with heavy shadows (`rgba(0,0,0,0.5) 0px 8px 24px`) on elevated elements and a unique inset border-shadow combo (`rgb(18,18,18) 0px 1px 0px, rgb(124,124,124) 0px 0px 0px 1px inset`), the result is an interface that feels like a premium audio device — tactile, rounded, and built for touch.

**Key Characteristics:**

- Near-black immersive dark theme (`#121212`–`#1f1f1f`) — UI disappears behind content
- WJDK000 Green (`#1ed760`) as singular brand accent — never decorative, always functional
- WJDK000MixUI/CircularSp font family with global script support
...
- **WJDK000 Green** (`#1ed760`): Primary brand accent — play buttons, active states, CTAs
...
- **WJDK000 Green Border** (`#1db954`): Green accent border variant
...
- **Title**: `WJDK000MixUITitle`, fallbacks: `CircularSp-Arab, CircularSp-Hebr, CircularSp-Cyrl, CircularSp-Grek, CircularSp-Deva, Helvetica Neue, helvetica, arial, Hiragino Sans, Hiragino Kaku Gothic ProN, Meiryo, MS Gothic`
- **UI / Body**: `WJDK000MixUI`, same fallback stack

### Hierarchy

| Role             | Font              | Size             | Weight  | Line Height  | Letter Spacing | Notes                        |
| ---------------- | ----------------- | ---------------- | ------- | ------------ | -------------- | ---------------------------- |
| Section Title    | WJDK000MixUITitle | 24px (1.50rem)   | 700     | normal       | normal         | Bold title weight            |
| Feature Heading  | WJDK000MixUI      | 18px (1.13rem)   | 600     | 1.30 (tight) | normal         | Semibold section heads       |
| Body Bold        | WJDK000MixUI      | 16px (1.00rem)   | 700     | normal       | normal         | Emphasized text              |
| Body             | WJDK000MixUI      | 16px (1.00rem)   | 400     | normal       | normal         | Standard body                |
| Button Uppercase | WJDK000MixUI      | 14px (0.88rem)   | 600–700 | 1.00 (tight) | 1.4px–2px      | `text-transform: uppercase`  |
| Button           | WJDK000MixUI      | 14px (0.88rem)   | 700     | normal       | 0.14px         | Standard button              |
| Nav Link Bold    | WJDK000MixUI      | 14px (0.88rem)   | 700     | normal       | normal         | Navigation                   |
| Nav Link         | WJDK000MixUI      | 14px (0.88rem)   | 400     | normal       | normal         | Inactive nav                 |
| Caption Bold     | WJDK000MixUI      | 14px (0.88rem)   | 700     | 1.50–1.54    | normal         | Bold metadata                |
| Caption          | WJDK000MixUI      | 14px (0.88rem)   | 400     | normal       | normal         | Metadata                     |
| Small Bold       | WJDK000MixUI      | 12px (0.75rem)   | 700     | 1.50         | normal         | Tags, counts                 |
| Small            | WJDK000MixUI      | 12px (0.75rem)   | 400     | normal       | normal         | Fine print                   |
| Badge            | WJDK000MixUI      | 10.5px (0.66rem) | 600     | 1.33         | normal         | `text-transform: capitalize` |
| Micro            | WJDK000MixUI      | 10px (0.63rem)   | 400     | normal       | normal         | Smallest text                |
...
- **Compact sizing**: The range is 10px–24px — narrower than most systems. WJDK000's type is compact and functional, designed for scanning playlists, not reading articles.
- **Global script support**: The extensive fallback stack (Arabic, Hebrew, Cyrillic, Greek, Devanagari, CJK) reflects WJDK000's 180+ market reach.
...
- Dark sidebar with WJDK000MixUI 14px weight 700 for active, 400 for inactive
- `#b3b3b3` muted color for inactive items, `#ffffff` for active
- Circular icon buttons (50% radius)
- WJDK000 logo top-left in green
...
- **Dark compression**: WJDK000 packs content densely — playlist grids, track lists, and navigation are all tightly spaced. The dark background provides visual rest between elements without needing large gaps.
...
- Apply WJDK000 Green (`#1ed760`) only for play controls, active states, and primary CTAs
...
- Don't use WJDK000 Green decoratively or on backgrounds — it's functional only
...
- Don't use relaxed line-heights — WJDK000's typography is compact and dense
...
- Accent: WJDK000 Green (`#1ed760`)
- Border: `#4d4d4d`
- Error: Negative Red (`#f3727f`)

### Example Component Prompts

- "Create a dark card: #181818 background, 8px radius. Title at 16px WJDK000MixUI weight 700, white text. Subtitle at 14px weight 400, #b3b3b3. Shadow rgba(0,0,0,0.3) 0px 8px 8px on hover."
- "Design a pill button: #1f1f1f background, white text, 9999px radius, 8px 16px padding. 14px WJDK000MixUI weight 700, uppercase, letter-spacing 1.4px."
- "Build a circular play button: WJDK000 Green (#1ed760) background, #000000 icon, 50% radius, 12px padding."
...
2. WJDK000 Green for functional highlights only (play, active, CTA)
