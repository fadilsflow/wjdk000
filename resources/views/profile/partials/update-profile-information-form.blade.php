<section>
    <header>
        <h2 class="text-lg font-bold text-[color:var(--color-text)]">
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-[color:var(--color-text)]">
                        ...
                        <button form="send-verification" class="underline text-sm text-[color:var(--color-text-muted)] hover:text-[color:var(--color-text)] rounded-md focus:outline-none">
                            {{ __('Click here to re-send the verification email.') }}
                        </button>
                    </p>

                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-medium text-sm text-[color:var(--color-brand)]">
