...
if (!SECRET_TOKEN) {
    console.error('[FATAL] GATEWAY_SECRET_TOKEN tidak diset di .env. Server tidak bisa jalan.');
    process.exit(1);
}
...
function authMiddleware(req, res, next) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ success: false, message: 'Unauthorized: Token tidak ada.' });
    }
...
app.get('/health', (req, res) => {
    res.json({
        ...
        message: isReady ? 'Gateway aktif dan WhatsApp terhubung.' : 'Gateway aktif tapi WhatsApp belum scan QR.',
    });
});
...
app.post('/send', authMiddleware, async (req, res) => {
    if (!isReady) {
        return res.status(503).json({ success: false, message: 'WhatsApp belum siap. Scan QR Code terlebih dahulu.' });
    }

    const { to, message } = req.body;

    if (!to || typeof to !== 'string' || to.trim() === '') {
        return res.status(400).json({ success: false, message: 'Parameter "to" wajib diisi.' });
    }

    if (!message || typeof message !== 'string' || message.trim() === '') {
        return res.status(400).json({ success: false, message: 'Parameter "message" wajib diisi.' });
    }
    ...
    try {
        ...
        if (!numberId) {
            console.warn(`[WA GATEWAY] Nomor tidak terdaftar di WhatsApp: ${to} (formatted: ${phone})`);
